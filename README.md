...
| ไฟล์งาน | Google Drive (OAuth, google/apiclient) — แนบไฟล์หลายไฟล์/งาน |
...
- ส่งงานด้วยการ **แนบไฟล์** (อัปขึ้น Google Drive อัตโนมัติ — หลายไฟล์/งาน, ≤20MB/ไฟล์)
- **ดูคะแนนตัวเองไม่ได้** (เห็นแค่สถานะ "ส่งแล้ว")

### การเก็บไฟล์บน Google Drive

ไฟล์ทุกชิ้นถูกอัปขึ้น Drive ของบัญชีที่อาจารย์เชื่อมไว้ จัดเป็นโครงสร้าง:
```
[ราก] / วิชา / ห้อง / งาน / รหัสนักศึกษา / ไฟล์...
```
ตั้งค่าครั้งเดียว: `/admin/drive` → เชื่อมต่อ Google Drive (ต้องตั้ง `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET` ใน `.env` ก่อน — ดูวิธีในหน้านั้น)
