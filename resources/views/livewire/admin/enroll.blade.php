        ...
        <div class="card overflow-hidden">
            <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-2 ml-auto">
                    <select wire:model.live="roomFilter" class="select !w-auto">
                        <option value="">ทุกห้อง</option>
                        @foreach ($rooms as $r)<option value="{{ $r->id }}">{{ $r->name }}</option>@endforeach
                    </select>
                    @if ($roomFilter)
                        <button wire:click="enrollRoom" class="btn-primary btn-sm"><x-icon name="plus" class="w-4 h-4" /> ลงทะเบียนทั้งห้อง</button>
                    @endif
                </div>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead class="bg-slate-50 dark:bg-slate-800/50">
                        <tr><th class="th w-12"></th><th class="th">รหัส</th><th class="th">ชื่อ-สกุล</th><th class="th">ห้อง</th></tr>
                    </thead>
