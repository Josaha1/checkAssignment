<div class="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-brand-600 via-brand-600 to-violet-700 relative overflow-hidden">
    ...
    <div class="w-full max-w-sm relative">
        <div class="text-center mb-6">
            ...
            <p class="text-brand-100 text-sm mt-1">เข้าสู่ระบบด้วยรหัสนักศึกษาและวันเกิด</p>
        </div>
        ...
        <form wire:submit="login" class="card card-pad space-y-4">
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="text" wire:model="student_code" autofocus class="input pl-9" placeholder="เช่น 670012345">
                </div>
            </div>
            <div>
                <label class="label">วันเกิด <span class="text-slate-400 font-normal">(รหัสผ่าน)</span></label>
                <input type="date" wire:model="birthdate" class="input">
                <p class="text-xs text-slate-400 mt-1">วัน/เดือน/ปี ค.ศ. ตามทะเบียน</p>
            </div>

            @error('student_code')<p class="flex items-center gap-1.5 text-sm text-rose-600"><x-icon name="alert" class="w-4 h-4" /> {{ $message }}</p>@enderror
            @error('birthdate')<p class="flex items-center gap-1.5 text-sm text-rose-600"><x-icon name="alert" class="w-4 h-4" /> {{ $message }}</p>@enderror
